    ...
    const normalizedSamagri = {
      ...
      pujaKit:
        Array.isArray(
          ...
          ? pujaSamagri.pujaKit.filter(Boolean)
          : [],
      ...
      instantKit:
        Array.isArray(
          ...
          ? pujaSamagri.instantKit.filter(Boolean)
          : [],
    };
...
const bookingPopulate = [
  ...
  },
  {
    path: "vendor",
    select:
      "firstName lastName email phone image about experience language gotra",
  },
  {
    path: "pujaSamagri.pujaKit",
    select: "name",
  },
  {
    path: "pujaSamagri.instantKit",
    select: "name",
  },
];
...
const formatBooking = (booking) => {
  ...
  item.pujaSamagri = {
    pujaKit:
      item?.pujaSamagri?.pujaKit?.map((kit) => kit?.name) || [],
    instantKit:
      item?.pujaSamagri?.instantKit?.map((kit) => kit?.name) || [],
  };
  ...
  try {
    ...
    if (["admin", "super-admin"].includes(role)) {
      ...
      return res.status(200).json({
        ...
        payload: {
          bookings: allBookings.map(formatBooking),
        },
      });
    }
    ...
    if (type === "ongoing") {
      const potentialOngoing = await Booking.find({
        ...ownerField,
        status: {
          $in: ["pending", "upcoming", "ongoing", "accept"],
        },
        dateTime: { $lte: now },
      })
        .populate(bookingPopulate)
      ...
      const ongoing = potentialOngoing
        .filter((booking) => {
    ...
    if (type === "upcoming") {
      ...
      return res.status(200).json({
        ...
        payload: {
          bookings: upcoming.map(formatBooking),
        },
      });
    }
    ...
    if (type === "pending") {
      ...
      return res.status(200).json({
        ...
        payload: {
          bookings: pending.map(formatBooking),
        },
      });
    }
    ...
    if (type === "previous") {
      ...
      return res.status(200).json({
        ...
        payload: {
          bookings: previous.map(formatBooking),
        },
      });
    }
    ...
    if (type === "cancelled") {
      ...
      return res.status(200).json({
        ...
        payload: {
          bookings: cancelled.map(formatBooking),
        },
      });
    }
    ...
    activeBookings.forEach((booking) => {
      ...
      if (booking.status === "pending") {
        ...
        if (
          ...
          ongoing.push(formatBooking(booking));
    ...
    return res.status(200).json({
      ...
      payload: {
        ...
        pending,
        upcoming,
        previous: previous.map(formatBooking),
      },
    });
...
const getAllServices = async (req, res) => {
  try {
    ...
    const servicesWithVendor = await Service.find(
      ...
      .populate("vendor", "firstName lastName email phone role status");

    // Hide services whose vendor is missing or inactive (public view only,
    // admins/vendor-self views already scope the query above)
    const visibleServices =
      panditId || user.role === "vendor" || ["admin", "super-admin"].includes(user.role)
        ? servicesWithVendor
        : servicesWithVendor.filter(
          (service) => service.vendor && service.vendor.status === "active"
        );
      ...
      payload: {
        services: visibleServices.map((service) => {
          const matchedService = allServices.find(
            (item) => item.name.toLowerCase() === service.poojaType.toLowerCase()
          );
          ...
          return {
            ...
            price: service.price,
            views: randomViews,
            originalPrice: Math.ceil(service.price * 1.10),
            gst: 18,
            advance: 30,
            image: matchedService?.image?.url || "https://adhyatmah.com/images/poojaas/satyanarayan-puja.jpeg",
            vendor: {
...
const getPanditProfile = async (req, res) => {
  try {
    ...
    const vendor = await User.findById(vendorId)
      .select(
        "firstName lastName email image gender language phone about cover address city zip country state experience role status services gotra veda pankti shakha sutra pravar aadhar dateOfBirth experience, referral_code"
      )
      .populate({
        path: "services",
        select: "poojaType description duration price",
    ...
    // Hide the vendor from public profile view if it doesn't exist,
    // isn't a vendor, or is inactive
...
const getPanditList = async (req, res) => {
  try {
    ...
    // Only show active vendors/pandits in the public list
    let query = {
      role: "vendor",
      status: "active",
    };
    ...
    // Apply service filter BEFORE pagination
    if (serviceName) {
      const matchingServices = await Service.find({
        poojaType: {
          $regex: escapeRegex(serviceName),
          $options: "i",
        },
      }).select("_id");
    ...
    let formattedVendors = vendors.map((vendor) => {
      const vendorSlug = `${vendor.firstName || ""} ${vendor.lastName || ""
        }`
        .toLowerCase()
        .replace(/\s+/g, "-")
        .trim();
        ...
        services: vendor.services
          ? vendor.services.map((service) => {
            ...
            return {
              ...
              price: service.price,
              originalPrice:
                matchedService?.originalPrice || null,
              image:
                matchedService?.image?.url || null,
            };
          })
          : [],
    ...
    if (slug) {
      formattedVendors = formattedVendors.filter(
  ...
  try {
    const services = await Service.find({})
      .populate({
        path: "vendor",
        select:
          "firstName lastName email phone role status cover about experience language gotra veda pankti shakha sutra address city state country zip",
        match: { role: "vendor", status: "active" },
      })
      .sort({ createdAt: -1 });

    // vendor null wale hata do (isme inactive vendors bhi ab null aa jaenge
    // populate match ki wajah se, isliye ye filter unhe bhi hide kar dega)
...
const getPanditServices = async (req, res) => {
  try {
    ...
    // Inactive vendors ko public pandit-services page se bhi hide karo
    if (!vendor || vendor.role !== "vendor" || vendor.status !== "active") {
      return res.status(404).json({
        error: true,
        code: 404,
      ...
      payload: {
        booking: {
          ...
          vendor: {
            ...
            about:
              booking.vendor?.about,
            image:
              booking.vendor?.image,
            experience:
              booking.vendor?.experience,
            language:
              booking.vendor?.language,
            gotra:
              booking.vendor?.gotra,
            referral_code:
              booking.vendor?.referral_code,
          },
