...
import {
  EMPTY_KIT_DATA,
  buildPujaSamagriPayload,
  fetchServiceKits
} from 'src/utils/puja-kit-product';
import {
  MdAccessTime,
  MdExpandMore,
  MdShoppingBag,
...
export default function VendorServiceBookingPage() {
  ...
  useEffect(() => { setKitImageBroken(false); }, [kitImageUrl]);
  useEffect(() => { setInstantKitImageBroken(false); }, [instantKitImageUrl]);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (addr) {
          ...
          setState(addr.state || '');
          setCountry('India');
          if (!city) setCity(addr.city || addr.town || addr.village || addr.suburb || '');
          setZipLoading(false);
          return;
        }
      }
  ...
  const grandTotal = payableAmount + DELIVERY_CHARGE + HANDLING_CHARGE + SMALL_CART_CHARGE + poojaKitTotal + instantKitTotal;
  ...
  const KitQtyControls = ({ count, onDecrement, onIncrement, accentColor, borderColor }) => (
    <Stack direction="row" alignItems="center" sx={{ border: `1.5px solid ${borderColor}`, borderRadius: 2, overflow: 'hidden' }}>
      <Box onClick={onDecrement} sx={{ px: 1.25, py: 0.5, cursor: 'pointer', display: 'flex', alignItems: 'center', color: accentColor, '&:hover': { bgcolor: `${accentColor}18` } }}>
        <MdRemove size={16} />
      </Box>
      <Typography variant="body2" fontWeight={700} sx={{ px: 1.5, color: '#1a1a1a', minWidth: 24, textAlign: 'center' }}>
        {count}
      </Typography>
      <Box onClick={onIncrement} sx={{ px: 1.25, py: 0.5, cursor: count >= MAX_KIT_LIMIT ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', color: count >= MAX_KIT_LIMIT ? '#ccc' : accentColor, '&:hover': { bgcolor: count >= MAX_KIT_LIMIT ? 'transparent' : `${accentColor}18` } }}>
        <MdAdd size={16} />
      </Box>
    </Stack>
  );
                ...
                {poojaKitSkipped ? (
                  ...
                  >
                    ...
                    <Button
                      ...
                      size="small"
                      sx={{
                        borderRadius: '50px', px: '14px', py: '5px',
                        bgcolor: '#E87D2B', color: '#fff', textTransform: 'none', fontSize: 12,
                  ...
                  >
                    ...
                    >
                      <Box
                        ...
                        sx={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block', transition: 'transform .35s ease', '&:hover': { transform: 'scale(1.08)' } }}
                      />
                    </Box>
                    ...
                    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
                      <Box
                        sx={{
                          ...
                          px: '10px', py: '4px', bgcolor: '#FEF3E2', color: '#A05A00',
                          border: '0.5px solid #F0B840', borderRadius: '6px', width: 'fit-content'
                        }}
                      >
                      ...
                      <Stack direction="row" alignItems="center" flexWrap="wrap" gap="10px" sx={{ mt: 0.5 }}>
                        <Typography sx={{ fontSize: 17, fontWeight: 700 }}>
                          ₹{poojaKit?.price}
                          <Box component="span" sx={{ fontSize: 11, fontWeight: 400, color: 'text.secondary', ml: '3px' }}>/ Kit</Box>
                        </Typography>
                        ...
                        {poojaKitCount === 0 ? (
                          <Button
                            ...
                            size="small"
                            sx={{ borderRadius: '50px', px: '14px', py: '6px', bgcolor: '#E87D2B', color: '#fff', textTransform: 'none', fontSize: 13, '&:hover': { bgcolor: '#d06b20' } }}
                        ...
                        <Button
                          ...
                          variant="outlined"
                          sx={{
                            borderRadius: '50px', px: '12px', py: '5px',
                      ...
                      {poojaKitCount >= MAX_KIT_LIMIT && (
                        <Typography variant="caption" sx={{ color: '#888' }}>Maximum {MAX_KIT_LIMIT} kits allowed.</Typography>
                      )}
                ...
                {instantKitSkipped ? (
                  ...
                  >
                    ...
                    <Button
                      ...
                      sx={{
                        borderRadius: '50px', px: '14px', py: '5px',
                        bgcolor: '#3A9E28', color: '#fff', textTransform: 'none', fontSize: 12,
                    ...
                    <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
                      <Box
                        sx={{
                          display: 'inline-flex', alignItems: 'center', gap: '6px',
                          fontSize: 10, fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase',
                          px: '10px', py: '4px', bgcolor: '#E8F5E4', color: '#2D6A1A',
                          border: '0.5px solid #6DB95A', borderRadius: '6px', width: 'fit-content'
                        }}
                      >
                      ...
                      <Stack direction="row" alignItems="center" flexWrap="wrap" gap="10px" sx={{ mt: 0.5 }}>
                        ...
                        {instantKitCount === 0 ? (
                          <Button
                            ...
                            size="small"
                            sx={{ borderRadius: '50px', px: '14px', py: '6px', bgcolor: '#3A9E28', color: '#fff', textTransform: 'none', fontSize: 13, '&:hover': { bgcolor: '#2F7F20' } }}
                            startIcon={<MdAdd size={14} />}
                          >
                            Add
                          </Button>
          ...
          <Grid size={{ xs: 12, md: 6 }}>
            <Card>
              <CardContent>
                ...
                />
                <Popover
                  open={Boolean(timeAnchorEl)} anchorEl={timeAnchorEl}
                ...
                <Grid container spacing={2} sx={{ mb: 2 }}>
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <TextField
                      ...
                      placeholder="Enter 6-digit pincode" error={Boolean(zipError)}
                      ...
                      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ py: 0.75 }}>
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          <Typography variant="body2" sx={{ color: '#555' }}>
                            Puja Kit <Typography component="span" variant="caption" sx={{ color: '#f97316' }}>(×{poojaKitCount})</Typography>
                          </Typography>
                        </Stack>
                    ...
                    {instantKitCount > 0 && !instantKitSkipped && (
                      <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ py: 0.75 }}>
                        <Stack direction="row" alignItems="center" spacing={1}>
                          ...
                          <Typography variant="body2" sx={{ color: '#555' }}>
                            Instant Kit <Typography component="span" variant="caption" sx={{ color: '#6366f1' }}>(×{instantKitCount})</Typography>
                          </Typography>
                        </Stack>
            ...
            addons: {
              poojaKit:
                poojaKitCount > 0 && !poojaKitSkipped && poojaKit.id
                  ? { productId: poojaKit.id, name: poojaKit.name, price: poojaKit.price, quantity: poojaKitCount }
                  : null,
              instantKit:
                instantKitCount > 0 && !instantKitSkipped && instantKit.id
                  ? { productId: instantKit.id, name: instantKit.name, price: instantKit.price, quantity: instantKitCount }
                  : null
            }
          }}
