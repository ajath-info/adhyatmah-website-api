...
export default function SortBar({ productData, isLoading, sortData, filters }) {
  ...
  // limit change hone par page=1 reset karo taki sahi products dikhe
  const createLimitQueryString = useCallback(
    (limitValue) => {
      const params = new URLSearchParams(searchParams);
      params.set('limit', limitValue);
      params.delete('page'); // page reset karo
      return params.toString();
    },
    [searchParams]
  );
          ...
          <FormControl size="small" fullWidth sx={{ maxWidth: 120 }}>
            <Select
              ...
              onChange={(e) => {
                ...
                router.push(`${pathname}?${createLimitQueryString(newLimit)}`);
              }}
              sx={{
                '& .MuiSelect-select': {
                  textTransform: 'capitalize'
