...
export default function ServiceDetailsReviewForm({ ...props }) {
    const { onClose, sid, onClickCancel, onAddingReview, ...other } = props;
    ...
    const { mutate, isPending: isLoading } = useMutation({
        mutationFn: api.addServiceReview,
        onSuccess: ({ data, user }) => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async () => {
            mutate({ rating: values.rating, review: values.review, images: values.images.map((v) => v.url), sid: sid });
        }
    });
...
ServiceDetailsReviewForm.propTypes = {
